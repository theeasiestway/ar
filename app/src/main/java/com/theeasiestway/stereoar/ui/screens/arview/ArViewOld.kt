...
@Composable
fun CreateArView2(viewModel: ArViewModel) {
...
fun getLastVrMode2(context: Context): Boolean {
    val sp = context.getSharedPreferences("app_prefs", Context.MODE_PRIVATE)
    return sp.getBoolean("vr_mode", false)
}

fun setLastVrMode2(context: Context, vrMode: Boolean) {
    val sp = context.getSharedPreferences("app_prefs", Context.MODE_PRIVATE)
    sp.edit().putBoolean("vr_mode", vrMode).apply()
}

@Composable
fun getFootprintModel2(viewModel: ArViewModel): ModelRenderable? {
...
fun toastError2(context: Context, @StringRes message: Int, error: Throwable? = null) {
...
@Composable
fun ModelsList2(arModels: List<String>, onItemClicked: (String) -> Unit) {
    LazyColumn(
        modifier = Modifier
            .fillMaxWidth()
            .height(300.dp),
...
private fun RequestCameraPermission(
    ...
    else if (cameraPermissionState.status.shouldShowRationale) onCanBeRequested {
        cameraPermissionState.launchPermissionRequest()
    } else onCanNotBeRequested()
